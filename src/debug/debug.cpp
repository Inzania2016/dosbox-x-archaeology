...
static bool TraceOpen(const char* path);
static void TraceClose(void);
static void TraceAction(const char* label);
static void TraceSnap(const char* label);
...
static bool traceHasPrev = false;
static uint8_t tracePrev5C5A[0x100];
static uint8_t tracePrev5000[0x400];
static uint8_t tracePrev53EA[0x08];
static uint8_t tracePrev535C[0x01];
static uint8_t tracePrev53E0[0x02];
static uint8_t tracePrevSuspects[27];
static bool traceCodeHitLatched = false;
static uint16_t traceCodeHitLatchedCS = 0;
...
static const TraceSuspectWatch traceSuspects[] = {
    {"52C8", 0x52C8}, {"52CC", 0x52CC}, {"52CE", 0x52CE}, {"5350", 0x5350}, {"5356", 0x5356},
    {"535A", 0x535A}, {"5362", 0x5362}, {"5363", 0x5363}, {"5386", 0x5386}, {"5388", 0x5388},
    {"538A", 0x538A}, {"538C", 0x538C}, {"538E", 0x538E}, {"5394", 0x5394}, {"5396", 0x5396},
    {"5398", 0x5398}, {"539A", 0x539A}, {"53AA", 0x53AA}, {"53AB", 0x53AB}, {"53EA", 0x53EA},
    {"53EB", 0x53EB}, {"53EC", 0x53EC}, {"53ED", 0x53ED}, {"53EE", 0x53EE}, {"53EF", 0x53EF},
    {"53F0", 0x53F0}, {"53F1", 0x53F1}
};
...
bool ChangeRegister(char* const str)
{
    ...
    while(*hex) {
        ...
        if(strstr(hex, "EFLAGS") == hex) { hex += 6; CPU_SetFlags(GetHexValue(hex, hex), FMASK_ALL); }
        else
            if(strstr(hex, "FLAGS") == hex) { hex += 5; CPU_SetFlags(GetHexValue(hex, hex), FMASK_ALL & 0xFFFFu); }
            else

                //             "IOPL"

                if(strncmp(hex, "EAX", 3) == 0) { hex += 3; reg_eax = GetHexValue(hex, hex); }
                else
                    if(strncmp(hex, "EBX", 3) == 0) { hex += 3; reg_ebx = GetHexValue(hex, hex); }
                    else
                        if(strncmp(hex, "ECX", 3) == 0) { hex += 3; reg_ecx = GetHexValue(hex, hex); }
                        else
                            if(strncmp(hex, "EDX", 3) == 0) { hex += 3; reg_edx = GetHexValue(hex, hex); }
                            else
                                if(strncmp(hex, "ESI", 3) == 0) { hex += 3; reg_esi = GetHexValue(hex, hex); }
                                else
                                    if(strncmp(hex, "EDI", 3) == 0) { hex += 3; reg_edi = GetHexValue(hex, hex); }
                                    else
                                        if(strncmp(hex, "EBP", 3) == 0) { hex += 3; reg_ebp = GetHexValue(hex, hex); }
                                        else
                                            if(strncmp(hex, "ESP", 3) == 0) { hex += 3; reg_esp = GetHexValue(hex, hex); }
                                            else
                                                if(strncmp(hex, "EIP", 3) == 0) { hex += 3; reg_eip = GetHexValue(hex, hex); }
                                                else

                                                    if(strncmp(hex, "AX", 2) == 0) { hex += 2; reg_ax = (uint16_t)GetHexValue(hex, hex); }
                                                    else
                                                        if(strncmp(hex, "BX", 2) == 0) { hex += 2; reg_bx = (uint16_t)GetHexValue(hex, hex); }
                                                        else
                                                            if(strncmp(hex, "CX", 2) == 0) { hex += 2; reg_cx = (uint16_t)GetHexValue(hex, hex); }
                                                            else
                                                                if(strncmp(hex, "DX", 2) == 0) { hex += 2; reg_dx = (uint16_t)GetHexValue(hex, hex); }
                                                                else
                                                                    if(strncmp(hex, "SI", 2) == 0) { hex += 2; reg_si = (uint16_t)GetHexValue(hex, hex); }
                                                                    else
                                                                        if(strncmp(hex, "DI", 2) == 0) { hex += 2; reg_di = (uint16_t)GetHexValue(hex, hex); }
                                                                        else
                                                                            if(strncmp(hex, "BP", 2) == 0) { hex += 2; reg_bp = (uint16_t)GetHexValue(hex, hex); }
                                                                            else
                                                                                if(strncmp(hex, "SP", 2) == 0) { hex += 2; reg_sp = (uint16_t)GetHexValue(hex, hex); }
                                                                                else
                                                                                    if(strncmp(hex, "IP", 2) == 0) { hex += 2; reg_ip = (uint16_t)GetHexValue(hex, hex); }
                                                                                    else

                                                                                        if(strncmp(hex, "AL", 2) == 0) { hex += 2; reg_al = (uint8_t)GetHexValue(hex, hex); }
                                                                                        else
                                                                                            if(strncmp(hex, "BL", 2) == 0) { hex += 2; reg_bl = (uint8_t)GetHexValue(hex, hex); }
                                                                                            else
                                                                                                if(strncmp(hex, "CL", 2) == 0) { hex += 2; reg_cl = (uint8_t)GetHexValue(hex, hex); }
                                                                                                else
                                                                                                    if(strncmp(hex, "DL", 2) == 0) { hex += 2; reg_dl = (uint8_t)GetHexValue(hex, hex); }
                                                                                                    else

                                                                                                        if(strncmp(hex, "AH", 2) == 0) { hex += 2; reg_ah = (uint8_t)GetHexValue(hex, hex); }
                                                                                                        else
                                                                                                            if(strncmp(hex, "BH", 2) == 0) { hex += 2; reg_bh = (uint8_t)GetHexValue(hex, hex); }
                                                                                                            else
                                                                                                                if(strncmp(hex, "CH", 2) == 0) { hex += 2; reg_ch = (uint8_t)GetHexValue(hex, hex); }
                                                                                                                else
                                                                                                                    if(strncmp(hex, "DH", 2) == 0) { hex += 2; reg_dh = (uint8_t)GetHexValue(hex, hex); }
                                                                                                                    else

                                                                                                                        if(strncmp(hex, "CS", 2) == 0) { hex += 2; SegSet16(cs, (uint16_t)GetHexValue(hex, hex)); }
                                                                                                                        else
                                                                                                                            if(strncmp(hex, "DS", 2) == 0) { hex += 2; SegSet16(ds, (uint16_t)GetHexValue(hex, hex)); }
                                                                                                                            else
                                                                                                                                if(strncmp(hex, "ES", 2) == 0) { hex += 2; SegSet16(es, (uint16_t)GetHexValue(hex, hex)); }
                                                                                                                                else
                                                                                                                                    if(strncmp(hex, "FS", 2) == 0) { hex += 2; SegSet16(fs, (uint16_t)GetHexValue(hex, hex)); }
                                                                                                                                    else
                                                                                                                                        if(strncmp(hex, "GS", 2) == 0) { hex += 2; SegSet16(gs, (uint16_t)GetHexValue(hex, hex)); }
                                                                                                                                        else
                                                                                                                                            if(strncmp(hex, "SS", 2) == 0) { hex += 2; SegSet16(ss, (uint16_t)GetHexValue(hex, hex)); }
                                                                                                                                            else

                                                                                                                                                if(strncmp(hex, "AC", 2) == 0) { hex += 2; SETFLAGBIT(AC, GetHexValue(hex, hex)); }
                                                                                                                                                else
                                                                                                                                                    if(strncmp(hex, "AF", 2) == 0) { hex += 2; SETFLAGBIT(AF, GetHexValue(hex, hex)); }
                                                                                                                                                    else
                                                                                                                                                        if(strncmp(hex, "CF", 2) == 0) { hex += 2; SETFLAGBIT(CF, GetHexValue(hex, hex)); }
                                                                                                                                                        else
                                                                                                                                                            if(strncmp(hex, "DF", 2) == 0) { hex += 2; SETFLAGBIT(DF, GetHexValue(hex, hex)); }
                                                                                                                                                            else
                                                                                                                                                                if(strncmp(hex, "ID", 2) == 0) { hex += 2; SETFLAGBIT(ID, GetHexValue(hex, hex)); }
                                                                                                                                                                else
                                                                                                                                                                    if(strncmp(hex, "IF", 2) == 0) { hex += 2; SETFLAGBIT(IF, GetHexValue(hex, hex)); }
                                                                                                                                                                    else
                                                                                                                                                                        //             "NT"
                                                                                                                                                                        if(strncmp(hex, "OF", 2) == 0) { hex += 2; SETFLAGBIT(OF, GetHexValue(hex, hex)); }
                                                                                                                                                                        else
                                                                                                                                                                            if(strncmp(hex, "PF", 2) == 0) { hex += 2; SETFLAGBIT(PF, GetHexValue(hex, hex)); }
                                                                                                                                                                            else
                                                                                                                                                                                if(strncmp(hex, "SF", 2) == 0) { hex += 2; SETFLAGBIT(SF, GetHexValue(hex, hex)); }
                                                                                                                                                                                else
                                                                                                                                                                                    if(strncmp(hex, "TF", 2) == 0) { hex += 2; SETFLAGBIT(TF, GetHexValue(hex, hex)); }
                                                                                                                                                                                    else
                                                                                                                                                                                        //             "VM"
                                                                                                                                                                                        if(strncmp(hex, "ZF", 2) == 0) { hex += 2; SETFLAGBIT(ZF, GetHexValue(hex, hex)); }
                                                                                                                                                                                        else
                                                                                                                                                                                        {
                                                                                                                                                                                            return false;
                                                                                                                                                                                        }
...
bool ParseCommand(char* str) {
    std::string raw_copy_str = str;
    raw_copy_str += '\0'; /* paranoid */
    char* raw_found = &(raw_copy_str[0]);

    raw_found = trim(raw_found);
    string raw_s_found(raw_found);
    istringstream raw_stream(raw_s_found);
    string raw_command;
    raw_stream >> raw_command;
    string::size_type raw_next = raw_s_found.find_first_not_of(' ', raw_command.size());
    if(raw_next == string::npos) raw_next = raw_command.size();
    (raw_s_found.erase)(0, raw_next);
    raw_found = const_cast<char*>(raw_s_found.c_str());
    ...
    if(command == "TRACEOUT") {
        SkipSpace(raw_found);
        TraceOpen((*raw_found) ? raw_found : "TRACE.JSONL");
        return true;
    }

    if(command == "TRACECLOSE") {
        TraceClose();
        DEBUG_ShowMsg("DEBUG: Trace file closed.\n");
        return true;
    }

    if(command == "TRACEACTION") {
        SkipSpace(raw_found);
        TraceAction((*raw_found) ? raw_found : "ACTION");
        return true;
    }

    if(command == "TRACESNAP") {
        SkipSpace(raw_found);
        TraceSnap((*raw_found) ? raw_found : "SNAP");
        return true;
    }
    ...
    if(command == "HELP" || command == "?") {
        ...
        DEBUG_ShowMsg("TRACEOUT [file]           - Set JSONL trace output file.\n");
        DEBUG_ShowMsg("TRACEACTION [label]       - Write action marker to JSONL.\n");
        DEBUG_ShowMsg("TRACESNAP [label]         - Write watched snapshot to JSONL.\n");
        DEBUG_ShowMsg("TRACECLOSE                - Close active trace file.\n");
        ...
        DEBUG_ShowMsg("HOST+F6                   - Write a live trace snapshot to active trace file.\n");
        DEBUG_ShowMsg("TRACEOUT active + Ultima V action keys auto-log guest action snapshots.\n");
...
char* AnalyzeInstruction(char* inst, bool saveSelector) {
    ...
    if(pos) {
        ...
        if(!(get_tlb_readhandler(address)->flags & PFLAG_INIT)) {
            ...
            case 8: {
                uint8_t val = mem_readb(address);
                outmask[12] = '2';
                sprintf(result, outmask, prefix, adr, val);
            }	break;
            case 16: {
                uint16_t val = mem_readw(address);
                outmask[12] = '4';
                sprintf(result, outmask, prefix, adr, val);
            }	break;
            case 32: {
                uint32_t val = mem_readd(address);
                outmask[12] = '8';
                sprintf(result, outmask, prefix, adr, val);
            }	break;
            }
...
void DEBUG_Init() {
...

    {
        ...
        MAPPER_AddHandler(DEBUG_LiveTraceSnap_Handler, MK_f6, MMODHOST, "live_trace_snap", "Live trace snapshot", &item);
        if(item != NULL) {
            item->set_description("Write a live trace snapshot with a timestamp label when a trace file is active");
            item->set_text("Live trace snapshot");
        }
    }
...
static void TraceWriteEscaped(FILE* f, const char* s) {
    ...
    while(*s) {
        ...
        case '\t': fputs("\\t", f); break;
        default:
            if(c < 0x20) fprintf(f, "\\u%04X", (unsigned int)c);
            else fputc(c, f);
            break;
        }
    }
...
static uint32_t TraceReadBytes(uint16_t seg, uint32_t ofs, uint8_t* dst, uint32_t num) {
    ...
    for(uint32_t i = 0; i < num; i++) {
        uint8_t val = 0;
        if(!mem_readb_checked((PhysPt)GetAddress(seg, ofs + i), &val)) ok++;
        dst[i] = val;
    }
    return ok;
}
...
static void TraceWriteChangedOffsets(FILE* f, uint32_t baseOfs, const uint8_t* prev, const uint8_t* cur, uint32_t num, uint32_t max_items) {
    ...
    for(uint32_t i = 0; i < num; i++) {
        if(prev[i] != cur[i]) {
            if(emitted != 0) fputc(',', f);
            fprintf(f, "\"%04X\"", (unsigned int)(baseOfs + i));
            emitted++;
...
static void TraceWriteChangedSlots(FILE* f, const uint8_t* prev, const uint8_t* cur, uint32_t slot_size, uint32_t slot_count) {
    ...
    for(uint32_t slot = 0; slot < slot_count; slot++) {
        bool changed = false;
        uint32_t slot_base = slot * slot_size;
        for(uint32_t i = 0; i < slot_size; i++) {
            if(prev[slot_base + i] != cur[slot_base + i]) {
                changed = true;
                break;
            }
        }
...
static void TraceWriteChangedSlotRecords(FILE* f, const uint8_t* prev, const uint8_t* cur, uint32_t slot_size, uint32_t slot_count) {
    ...
    for(uint32_t slot = 0; slot < slot_count; slot++) {
        ...
        if(changed) {
            if(emitted != 0) fputc(',', f);
            fprintf(f, "{\"slot\":%u,\"before\":\"", (unsigned int)slot);
            TraceWriteHexBuffer(f, prev + slot_base, slot_size);
            fprintf(f, "\",\"after\":\"");
            TraceWriteHexBuffer(f, cur + slot_base, slot_size);
            fprintf(f, "\"}");
            emitted++;
        }
    }
...
static void TraceMakeLiveLabel(char* dst, size_t dst_size) {
    ...
    struct tm tm_now;
    unsigned int millis = (unsigned int)(SDL_GetTicks() % 1000u);
    ...
    snprintf(dst, dst_size, "LIVE_%04d%02d%02d_%02d%02d%02d_%03u",
        tm_now.tm_year + 1900,
        tm_now.tm_mon + 1,
        tm_now.tm_mday,
        tm_now.tm_hour,
...
static void TraceMakeActionLabel(const char* prefix, char* dst, size_t dst_size) {
    char stamp[64];
    const char* use_prefix = (prefix != NULL && *prefix != 0) ? prefix : "ACTION";
    const char* suffix = NULL;

    if(dst == NULL || dst_size == 0)
        return;

    TraceMakeLiveLabel(stamp, sizeof(stamp));
    suffix = stamp;
    if(strncmp(suffix, "LIVE_", 5) == 0)
        suffix += 5;
...
void DEBUG_TraceGuestAction(const char* action_label, const char* guest_key_name) {
    char label[128];
    const char* action = (action_label != NULL && *action_label != 0) ? action_label : "ACTION";
    const char* guest_key = (guest_key_name != NULL && *guest_key_name != 0) ? guest_key_name : "unknown";
    ...
    fprintf(traceFile, "\",\"guest_key\":\"");
    TraceWriteEscaped(traceFile, guest_key);
    fprintf(traceFile, "\"}\n");
    fflush(traceFile);
...
static void DEBUG_LiveTraceSnap_Handler(bool pressed) {
    ...
    TraceMakeLiveLabel(label, sizeof(label));
    TraceSnap(label);
}
...
static bool TraceOpen(const char* path) {
    ...
    traceFile = fopen(path, "wt");
    if(!traceFile) {
        DEBUG_ShowMsg("DEBUG: Trace file open failed.\n");
        return false;
    }
    ...
    DEBUG_ShowMsg("DEBUG: Trace output set to %s\n", path);
    return true;
}

static void TraceClose(void) {
    if(traceFile != NULL) {
        fprintf(traceFile, "{\"event\":\"trace_close\"}\n");
        fflush(traceFile);
        fclose(traceFile);
        traceFile = NULL;
    }
...
static void TraceSnap(const char* label) {
    uint8_t cur5C5A[0x100];
    uint8_t cur5000[0x400];
    uint8_t cur53EA[0x08];
    uint8_t cur535C[0x01];
    uint8_t cur53E0[0x02];
    uint8_t curSuspects[27];
    size_t suspect_count = sizeof(traceSuspects) / sizeof(traceSuspects[0]);
    size_t i;
    ...
    TraceReadBytes(0x1787, 0x5C5A, cur5C5A, 0x100);
    TraceReadBytes(0x1787, 0x5000, cur5000, 0x400);
    ...
    for(i = 0; i < suspect_count; i++)
        curSuspects[i] = mem_readb(0x17870u + traceSuspects[i].ofs);

    fprintf(traceFile, "{\"event\":\"snapshot\",\"label\":\"");
    TraceWriteEscaped(traceFile, label);
    fprintf(traceFile, "\",\"regs\":{");
    fprintf(traceFile, "\"cs\":\"%04X\",", (unsigned int)SegValue(cs));
    fprintf(traceFile, "\"ip\":\"%08X\",", (unsigned int)reg_eip);
    fprintf(traceFile, "\"ds\":\"%04X\",", (unsigned int)SegValue(ds));
    fprintf(traceFile, "\"es\":\"%04X\",", (unsigned int)SegValue(es));
    fprintf(traceFile, "\"ss\":\"%04X\",", (unsigned int)SegValue(ss));
    fprintf(traceFile, "\"ax\":\"%04X\",", (unsigned int)reg_ax);
    fprintf(traceFile, "\"bx\":\"%04X\",", (unsigned int)reg_bx);
    fprintf(traceFile, "\"cx\":\"%04X\",", (unsigned int)reg_cx);
    fprintf(traceFile, "\"dx\":\"%04X\",", (unsigned int)reg_dx);
    fprintf(traceFile, "\"si\":\"%04X\",", (unsigned int)reg_si);
    fprintf(traceFile, "\"di\":\"%04X\",", (unsigned int)reg_di);
    fprintf(traceFile, "\"bp\":\"%04X\",", (unsigned int)reg_bp);
    fprintf(traceFile, "\"sp\":\"%04X\",", (unsigned int)reg_sp);
    fprintf(traceFile, "\"flags\":\"%08lX\"", (unsigned long)reg_flags);
    fprintf(traceFile, "},");

    fprintf(traceFile, "\"ranges\":{");
    fprintf(traceFile, "\"ram_5c5a\":\"");
    TraceWriteHexBuffer(traceFile, cur5C5A, 0x100);
    fprintf(traceFile, "\",");
    fprintf(traceFile, "\"ram_5000\":\"");
    TraceWriteHexBuffer(traceFile, cur5000, 0x400);
    fprintf(traceFile, "\",");
    fprintf(traceFile, "\"hot_53ea\":\"");
    TraceWriteHexBuffer(traceFile, cur53EA, 0x08);
    fprintf(traceFile, "\"},");

    fprintf(traceFile, "\"singletons\":{");
    fprintf(traceFile, "\"byte_535c\":\"");
    TraceWriteHexBuffer(traceFile, cur535C, 0x01);
    fprintf(traceFile, "\",");
    fprintf(traceFile, "\"word_53e0\":\"");
    ...
    for(i = 0; i < suspect_count; i++) {
        if(i != 0)
            fprintf(traceFile, ",");
        fprintf(traceFile, "\"%s\":%u", traceSuspects[i].label, (unsigned int)curSuspects[i]);
    }
    fprintf(traceFile, "},");
    ...
    if(traceHasPrev) {
        fprintf(traceFile, ",\"ram_5c5a_changed_bytes\":%lu", (unsigned long)TraceCountChanged(tracePrev5C5A, cur5C5A, 0x100));
        fprintf(traceFile, ",\"ram_5c5a_changed_slots\":");
        TraceWriteChangedSlots(traceFile, tracePrev5C5A, cur5C5A, 8, 0x100 / 8);
        fprintf(traceFile, ",\"ram_5c5a_slot_records\":");
        TraceWriteChangedSlotRecords(traceFile, tracePrev5C5A, cur5C5A, 8, 0x100 / 8);

        fprintf(traceFile, ",\"ram_5000_changed_bytes\":%lu", (unsigned long)TraceCountChanged(tracePrev5000, cur5000, 0x400));
        fprintf(traceFile, ",\"ram_5000_changed_offsets\":");
        TraceWriteChangedOffsets(traceFile, 0x5000, tracePrev5000, cur5000, 0x400, 64);

        fprintf(traceFile, ",\"hot_53ea_changed_bytes\":%lu", (unsigned long)TraceCountChanged(tracePrev53EA, cur53EA, 0x08));
        fprintf(traceFile, ",\"hot_53ea_changed_offsets\":");
        TraceWriteChangedOffsets(traceFile, 0x53EA, tracePrev53EA, cur53EA, 0x08, 8);
        ...
        fprintf(traceFile, ",\"suspectchanges\":{");
        {
            size_t emitted = 0;
            for(i = 0; i < suspect_count; i++) {
                if(tracePrevSuspects[i] == curSuspects[i])
                    continue;
                if(emitted != 0)
                    fprintf(traceFile, ",");
                fprintf(traceFile, "\"%s\":{\"before\":%u,\"after\":%u}",
                        traceSuspects[i].label,
                        (unsigned int)tracePrevSuspects[i],
                        (unsigned int)curSuspects[i]);
                emitted++;
            }
        }
        fprintf(traceFile, "}");
    ...
    memcpy(tracePrev5C5A, cur5C5A, sizeof(tracePrev5C5A));
    memcpy(tracePrev5000, cur5000, sizeof(tracePrev5000));
